...
class Firewall (EventMixin):
	...
	def _handle_PacketIn(self, event):
		...
		try:
			...
			print(packet.next.ttl)
	#		print(packet.next.next.srcport)
			
			
			if not packet.src in port_table_mac.keys() and not packet.next.srcip in port_table_mac.values():
				port_table_mac[packet.src] = packet.next.srcip #if the MAC address and the IP are not already in the dict, add it to the dict.
				port_table_ip[packet.next.srcip] = packet.src #create IP <-> MAC pairs in another port table
			elif packet.next.srcip != port_table_mac[packet.src]: #if the ip address does not match the one listed for that MAC address, block that flow
				msg = of.ofp_flow_mod()
				msg.priority = 10000
				msg.match.dl_src = packet.src
				#msg.actions.append()
				self.connection.send(msg)
				print("Blocking Spoofed IP")
			elif packet.src != port_table_ip[packet.next.srcip]: #if the ip address does not match the one listed for that MAC address, block that flow
				msg = of.ofp_flow_mod()
				msg.priority = 10000
				msg.match.nw_src = packet.next.src
				#msg.actions.append()
				self.connection.send(msg)
				print("Blocking Spoofed MAC")
			print(port_table_mac)
			print(port_table_ip)
